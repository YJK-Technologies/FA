...
const PORT = 5000;
app.use(cors());
app.use(bodyParser.json({ limit: "10mb" })); // Increase to 10MB or more if needed
  ...
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        ...
        return res.status(400).json({ error: `Image encoding failed: ${error.response?.data?.error || error.message}` });
      }
    }
...
app.post("/attendance", async (req, res) => {
  try {
    ...
    for (let row of result.recordset) {
      ...
      for (let storedEncoding of storedEncodings) {
        ...
        if (matchResponse.data.match) {
          ...
          return res.json({ message: `${type === "checkin" ? "Check-in" : "Check-out"} successful for ${empID}` });
        }
      }
    }
    ...
    const errorMessage = err?.response?.data?.error || err?.response?.data?.message || "Internal server error";
